...
export function WarehouseModule({ resource, session, onNavigate, onReportError }: { resource: WorkspaceResource; session: AuthSession; onNavigate?: (resource: WorkspaceResource) => void; onReportError?: (incident: SupportIncident) => void }) {
  ...
  const localizedColumns = useMemo(() => configuredColumns.map((column) => ({ ...column, label: t(column.label) })), [configuredColumns, t]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-panel"><span className="text-slate-500">{response ? <>{t("Mostrando")} <strong className="text-slate-700">{rows.length}</strong> {t("de")} <strong className="text-slate-700">{new Intl.NumberFormat(localeForLanguage[language]).format(response.total)}</strong> {t("registros")}</> : t("Preparando datos…")}</span><div className="flex items-center gap-3"><label className="flex items-center gap-2 text-xs text-slate-500">{t("Filas")}<select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); }} className="rounded border border-slate-200 px-2 py-1.5 text-sm"><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></label><span className="text-xs text-slate-500">{t("Página")} {response?.page ?? 1} {t("de")} {response?.totalPages ?? 1}</span><button type="button" disabled={page <= 1 || loading} onClick={() => setPage((value) => value - 1)} className="grid h-8 w-8 place-items-center rounded border border-slate-200 disabled:opacity-40" aria-label={t("Página anterior")}><ChevronLeft size={16} /></button><button type="button" disabled={!response || page >= response.totalPages || loading} onClick={() => setPage((value) => value + 1)} className="grid h-8 w-8 place-items-center rounded border border-slate-200 disabled:opacity-40" aria-label={t("Página siguiente")}><ChevronRight size={16} /></button></div></div></>}
